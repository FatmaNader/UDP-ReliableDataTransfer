...
public class ServerSR implements Runnable {
    ...
    public ServerSR(int client_port, int Server_port, String Filename, InetAddress IPAddress) {
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println("The user " + client_port + "  -----------------------------------------------------------------------------CLIENT PORT HERE :" + client_port);
            loadFile(Filename);
            System.out.println("The user " + client_port + "  packets needed to send:" + Integer.toString(packets_needed));
            byte initialize[] = new byte[8];
            ByteBuffer bx = ByteBuffer.allocate(4);
            bx.putInt(server_port);
            byte result1[] = new byte[4];
            result1 = bx.array();
            System.out.println("The user " + client_port + "  packets server port:" + server_port);
            ...
            System.out.println("The user " + client_port + " SEND FILE FINISHED");
            serverSocket.close();
    ...
    public static void loadFile(String filename) {
        ...
        System.out.println("The user " + client_port + "file lenngth" + file.length());
    ...
    public static void SendFile(DatagramSocket serverSocket) throws IOException {
        ...
        System.out.println("The user " + client_port + "SEND FILE BEGIN");
        ...
        while (PCKT_NO < packets_needed) {
            ...
            if (PCKT_NO <= windowBase + windowSize) {   // if pipeline is not full
                ...
                while (true) {
                    ...
                    if (ackReceived == 1) {
                        ...
                        if (ackSeq == windowBase + 1) {
                             for (int i = windowBase + 1; i < packets_needed; i++) {
        ...
        // continue to receive acknowledgements until all acknowledgements received
        ...
        while (!serverSocket.isClosed()) {
                for (int i = windowBase + 1; i < windowBase + 1 + windowSize; i++) {
                        if (i < packets_needed) {
                                    byte[] packetBytes = get_packet(i);
                                    DatagramPacket packetToSend = new DatagramPacket(packet_to_send, packet_to_send.length, IPAddress, client_port);
                                    try {
                                        if (!serverSocket.isClosed()) {
                                            serverSocket.send(packetToSend);
                                           
                                           System.out.println(" Packet timer timed out, so resent packet with sequence number: " + i);
                                        }
                                    } catch (IOException e) {
                                        e.printStackTrace();
                                    }
    ...
    public static int[] recieve_Ack(DatagramSocket serverSocket) throws SocketException, IOException {
       ...
       // serverSocket.setSoTimeout(10);
        ...
        if (receivePacket.getData()[0] == -1 || flag == false) {
